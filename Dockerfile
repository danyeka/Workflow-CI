...
RUN mvn --batch-mode dependency:copy -Dartifact=org.mlflow:mlflow-scoring:2.3.2:pom -DoutputDirectory=/opt/java 
RUN mvn --batch-mode dependency:copy -Dartifact=org.mlflow:mlflow-scoring:2.3.2:jar -DoutputDirectory=/opt/java/jars 
RUN cp /opt/java/mlflow-scoring-2.3.2.pom /opt/java/pom.xml
RUN cd /opt/java && mvn --batch-mode dependency:copy-dependencies -DoutputDirectory=/opt/java/jars 



                    COPY model_dir/logistic_regression_model /opt/ml/model
